...
case class DeConvLayerConfig(
  ...
  override def apply(): Block = blockType match {
    case dl.deConv1dLbl => build1d
    case dl.deConv2dLbl => build2d
    case _ => throw new UnsupportedOperationException(s"CNN model $blockType is not supported")
  }

  private def build2d: Block = {
    var _builder = Conv2dTranspose.builder.setKernelShape(kernelShape)
  ...
  private def build1d: Block = {
    var _builder = Conv1dTranspose.builder.setKernelShape(kernelShape)
    if (isValidShape(strideShape))
      _builder = _builder.optStride(strideShape)
    if (isValidShape(paddingShape))
      _builder = _builder.optPadding(paddingShape)
  ...
  override def toString: String =
    s"deconv_${getId}_(Kernel: $kernelShape, Stride: $strideShape, Padding: $paddingShape, $numFilters $bias)"
}

final object DeConvLayerConfig {
  def apply(
    blockType: String,
    kernelShape: Shape,
    strideShape: Shape,
    numFilters: Int,
    bias: Boolean): DeConvLayerConfig =
    new DeConvLayerConfig(blockType, kernelShape, strideShape, new Shape(-1, -1), numFilters, bias)
  ...
  def apply(
    ...
    numFilters: Int,
    bias: Boolean): DeConvLayerConfig =
    new DeConvLayerConfig(blockType, kernelShape, new Shape(-1, -1), new Shape(-1, -1), numFilters, bias)
