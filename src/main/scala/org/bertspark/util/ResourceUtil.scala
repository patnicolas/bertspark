...
private[bertspark] trait ResourceUtil {
  self =>
  ...
  final def getFsPath(fsResourceFileName: String): Option[String] = {
    ...
    if (res != null)
      ...
      log.error(s"Cannot getPath for $fsResourceFileName")
      None
    }
  }
  ...
  final def getFsPath(fsFileResourceName1: String, fsFileResourceName2: String): Option[(String, String)] = {
    ...
    if (res1 != null && res2 != null) Some((res1.getPath, res2.getPath))
      ...
      log.error(s"ERROR: IO, cannot get path for $fsFileResourceName1 or $fsFileResourceName2")
      None
    }
  }
...
/**
  ...
  */
private[bertspark] final object ResourceUtil {
  import org.slf4j.{Logger, LoggerFactory}
  import scala.util.Try

  final val log: Logger = LoggerFactory.getLogger("ResourceUitl")
  final val TestCompleted: Boolean = true
  ...
  def info(message: String): Unit =  log.info(s"\n$message")
  def warn(message: String): Unit =  log.warn(s"\n$message")
  def error(message: String): Unit =  log.error(s"\n$message")
  ...
  /**
    ...
    * @return absolute path for a resource file
    */
  final def getFsRelativePath(fsResourceFileName: String): Try[String] = Try {
      ...
      do {
        ...
        if (line != null && line.contains(separator)) {
          ...
          if(separatorIndex != -1) {
            ...
            log.error(s"Could not extract key value from $line for $fileName")
        }
      } while (line != null)
      Some(props)
    } else {
      log.error(s"Could not load the resource file $fileName")
      None
    }
  }
  catch {
  ...
  final def cleanse(
    ...
    try {
      ...
      case e: FileNotFoundException =>
        log.error(s"ERROR: IO, incorrect format ${e.getMessage}")
      case e: IOException =>
        log.error(s"ERROR: IO, incorrect format ${e.getMessage}")
    }
    finally {
      if (printWriter != null) {
        try (printWriter.close)
          ...
          case e: IOException =>
            log.error(s"Cleanse cannot close handle ${e.getMessage}")
        }
      }
    }
  }
