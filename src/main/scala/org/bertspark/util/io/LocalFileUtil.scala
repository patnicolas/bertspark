 ...
 * @version 0.1
 */
private[bertspark] final object LocalFileUtil {
  import scala.reflect.runtime.universe.TypeTag

  final val log: Logger = LoggerFactory.getLogger("LocalFileUtil")
  ...
  final object Load {
     ...
     *
     * @param fsResourceFileName
     * @return
     */
    final def resource(fsResourceFileName: String): Option[String] = {
      ...
      try {
        ...
        case e: IOException =>
          log.error(s"Load $fsResourceFileName with ${e.getMessage}")
          None
      }
      finally {
        try {
     ...
     * @return
     */
    final def local(fsFilename: String): Option[String] = {
      import scala.io.Source._
      ...
      try {
        ...
        while (lines.hasNext) {
          ...
          count += 1
          if (count % 2500 == 0)
            log.info(s"$count records have been loaded")
        }
        Some(collector.toSeq.mkString("\n"))
      }
      catch {
        case e: IOException =>
          log.error(s"$fsFilename with ${e.getMessage}")
          None
        case e: Exception =>
          log.error(s"$fsFilename with ${e.getMessage}")
          None
        ...
        try {
          if (source != null) {
            try {
              ...
              case e: IOException =>
                log.error(s"Failed to close $fsFilename")
                None
            }
          }
        }
    ...
    final def local[T: ClassTag](fsFilename: String, parse: String => T): Option[Array[T]] = local[T](fsFilename, parse, true)

    final def local[T: ClassTag](fsFilename: String, parse: String => T, header: Boolean): Option[Array[T]] =
      if (header)
    ...
    final def local[T: ClassTag](
      ...
      try {
        ...
        while (lines.hasNext) {
          ...
          if (filter.map(_(line)).getOrElse(true) && loadingWindow(from, to, count)) {
        ...
        case e: PatternSyntaxException =>
          log.error(s"$fsFilename ${e.getMessage}")
          None
        case e: IOException =>
          log.error(s"$fsFilename ${e.getMessage}")
          None
     ...
     */
    final def fs[T: ClassTag](fsPath: String, f: (String, String) => T): Option[Array[T]] = {
      import java.io.File
      import scala.io.Source._
          ...
          try {
            ...
            if (source != null) {
              try { source.close }
                ...
                case e: IOException =>
                  log.error(s"Failed to close $fsPath")
                  None
              }
            }
          }
     ...
     * @note The decoder is needed to avoid nio character set exception
     */
    final def fs[T: ClassTag](files: Array[File], f: (String, String) => T): Option[Array[T]] =
      if (files.nonEmpty) {
        import scala.io.Source._
    ...
    final def fs(fsPath: String): Option[Array[(String, String)]] = fs(fsPath, (id: String, note: String) => (id, note))

    final def fs(files: Array[File]): Option[Array[(String, String)]] = fs(files, (id: String, note: String) => (id, note))
  }
  ...
  final object Save {
     ...
     */
    final def local(fsFileName: String, content: String, append: Boolean = true): Boolean = {
  ...
  final object Json {
    ...
    def save[T](fsOutputFileName: String, data: T): Boolean = try {
      ...
      case e: IOException =>
        log.error(s"$fsOutputFileName with ${e.getMessage}")
        false
      case e: Exception =>
        log.error(s"$fsOutputFileName with ${e.getMessage}")
        false
